...
func _process_grab_object(event: InputEvent) -> void:
	...
	elif event.is_action_released(Keybinds.GRAB) \
...
func _process_move_object(_event: InputEvent) -> void:
	...
	elif Input.is_action_just_pressed(Keybinds.INTERACT) \
...
func _process_movement() -> void:
	...
	and not hud.is_text_chat_open():
		...
		float(Input.is_action_pressed(Keybinds.SPRINT)) \
			* current_frame.sprint_speed_modifier)

		var input_dir = \
			Input.get_vector(Keybinds.LEFT, Keybinds.RIGHT, Keybinds.FORWARD, Keybinds.BACKWARD)
...
func _process_text_chat() -> void:
	...
	if Input.is_action_just_pressed(Keybinds.TALK_ENTRY) \
...
func _process_camera_control(event: InputEvent) -> void:
	...
	and EventsManager.is_mouse_captured():
		if not (current_frame.held_object and Input.is_action_pressed(Keybinds.ROTATE)):
			...
			if current_frame.held_object \
			and Input.is_action_pressed(Keybinds.ROTATE):
